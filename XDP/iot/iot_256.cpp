...
struct auth_hdr {
  ...
  uint64_t randomnumber3;
  uint64_t randomnumber4;
  uint32_t hash;
  uint32_t identifier;
  uint64_t prTime;
}
...
//int rc = setsockopt(sockfd, SOL_SOCKET, SO_BINDTODEVICE, opt, len);
  ...
  //struct crPair crp[5];
  uint64_t r11,r12,r13,r14,r21,r22,r23,r24;
  int found = 0;
  int n = 0, i;
 ...
 while(true){
    ...
    uint64_t rndNum1 = ntohl(auth_ch -> randomnumber1); // RN1 is recieved
    
    uint64_t rndNum2 = ntohl(auth_ch -> randomnumber2); // RN1 is recieved
    uint64_t rndNum3 = ntohl(auth_ch -> randomnumber3); // RN1 is recieved
    uint64_t rndNum4 = ntohl(auth_ch -> randomnumber4); // RN1 is recieved
    ...
    //cout << "Challenge 2 recieved from Verifier is: "   << c2 << endl;
     ...
     for (i = 0; i < 500; i++) { // compute r1 and r2 from c1 and c2
      if ( c1 == uav_cr[clientCount].crp[i].ch1 && c2 == uav_cr[clientCount].crp[i].ch2) {
        r11 = uav_cr[clientCount].crp[i].resp11;
        r12 = uav_cr[clientCount].crp[i].resp12;
        r13 = uav_cr[clientCount].crp[i].resp13;
        r14 = uav_cr[clientCount].crp[i].resp14;
	r21 = uav_cr[clientCount].crp[i].resp21;
	r22 = uav_cr[clientCount].crp[i].resp22;
	r23 = uav_cr[clientCount].crp[i].resp23;
	r24 = uav_cr[clientCount].crp[i].resp24;
        found = 1;
        break;
      }
    }
    ...
    rndNum1 = (rndNum1 ^ (r11 | r21));
    
    rndNum2 = (rndNum2 ^ (r12 | r22));
    rndNum3 = (rndNum3 ^ (r13 | r23));
    rndNum4 = (rndNum4 ^ (r14 | r24));
    ...
    uint64_t input_ints[12];
        ...
        input_ints[6] = r23;
        input_ints[7] = r24;
        input_ints[8] = rndNum1;
        input_ints[9] = rndNum2;
        input_ints[10] = rndNum3;
        input_ints[11] = rndNum4;
    uint32_t Hash = computeHash(input_ints,12); 
    ...
    auth_response.randomnumber1= 0;
    auth_response.randomnumber2= 0;
    ...
    for(int i = 0; i < 500; i++)
       {
                	c1 = c1+4;
                        ...
                        uav_cr[j].crp[i].resp11 = r11;
                        uav_cr[j].crp[i].resp12 = r12;
                        uav_cr[j].crp[i].resp13 = r13;
                        uav_cr[j].crp[i].resp14 = r14;
                        uav_cr[j].crp[i].resp21 = r21;
                        uav_cr[j].crp[i].resp22 = r22;
                        uav_cr[j].crp[i].resp23 = r23;
                        uav_cr[j].crp[i].resp24 = r24;
     ...
     for(int i=0;i<2;i++)
        {
                ...
                for(int j=0;j<500;j++)
                {
               		cout<<uav_cr[i].crp[j].ch1<<" ";
               		 ...
               		 cout<<uav_cr[i].crp[j].resp11<<" ";
                	cout<<uav_cr[i].crp[j].resp21<<" ";
                	cout << endl;
