...
static uint32_t make_challenge_header(struct auth_hdr* payload)
{
	...
	uint32_t id = bpf_ntohl(payload->identifier)-1;
        ...
        uint64_t RN2 = bpf_get_prandom_u32();
        uint64_t RN3 = bpf_get_prandom_u32();
	...
	uint64_t r11=rec->resp11;
        ...
        uint64_t r13=rec->resp13;
        uint64_t r14=rec->resp14;
	uint64_t r21=rec->resp21;
    uint64_t r22=rec->resp22;
    uint64_t r23=rec->resp23;
    uint64_t r24=rec->resp24;

    uint64_t temp1 = r11 | r21;
    uint64_t temp2 = r12 | r22;
    uint64_t temp3 = r13 | r23;
    uint64_t temp4 = r14 | r24;
    uint64_t result1 = RN ^ temp1;
    uint64_t result2 = RN1 ^ temp2;
    uint64_t result3 = RN2 ^ temp3;
    uint64_t result4 = RN3 ^ temp4;
    ...
    payload->randomnumber3 = bpf_htonl(result3);
    payload->randomnumber4 = bpf_htonl(result4);
