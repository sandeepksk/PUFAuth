...
static void init_CR_pairs(int map_fd, __u32 map_type)
{
	...
	__u32 key = 0;
  	for (int i = 0; i < no_of_IOT; i++)
  	{
  		struct crPair uav_cr;
			...
			if(bpf_map_update_elem(map_fd,&key,&uav_cr,BPF_ANY) != 0)
       			{
       				fprintf(stderr,
	...
	for(i=0;i<2;i++)
	{
		printf("%d IOT\n",i+1);
		__u32 k=i*500;
		for(__u32 j=k;j<(k+500);j++)
		{
			struct crPair uav_cr;
			bpf_map_lookup_elem(map_fd,&j,&uav_cr);
			printf("%d %d %d %d\n",uav_cr.ch1,uav_cr.ch2,uav_cr.resp11,uav_cr.resp21);
		}
		printf("\n\n\n");
	}
	printf("Sucessful INIT of CR DB with total %d CR pairs\n",key);	
}
...
void init_crc32_table(int map_fd) {
    __u32 i, j, crc;
	...
	if(bpf_map_update_elem(map_fd,&i,&crc,BPF_ANY) != 0)
       	{
       		fprintf(stderr,
			"ERR: bpf_map_update_elem failed key:0x%X\n",i);
       	}		
    ...
    printf("Successful init of Hash  table for CRC32\n");
}
...
int main(int argc, char **argv)
{
	...
	map_expect.max_entries = total_no_of_crps;
	...
	/* Lesson#3: Locate map file descriptor */
	int crc32__map_fd = find_map_fd(bpf_obj, "crc32_table");
	if (crc32__map_fd < 0) {
		xdp_link_detach(cfg.ifindex, cfg.xdp_flags, 0);
		return EXIT_FAIL_BPF;
	}
	...
	map_expect.max_entries = hashcrc32size;
	err = __check_map_fd_info(crc32__map_fd, &info, &map_expect);
	if (err) {
		fprintf(stderr, "ERR: map via FD not compatible\n");
		return err;
	}
	if (verbose) {
		printf("\nCollecting stats from BPF crc32 map\n");
		printf(" - BPF map (bpf_map_type:%d) id:%d name:%s"
		       " key_size:%d value_size:%d max_entries:%d\n",
		       info.type, info.id, info.name,
		       info.key_size, info.value_size, info.max_entries
	...
	init_crc32_table(crc32__map_fd);
