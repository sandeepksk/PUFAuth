...
class EnrollmentTools(threading.Thread):
    ...
    def run(self):
        ...
        zero_map = {key: 0 for key in strong_zeros[:2331]}

        CRPs = {}

        for challenge_map_index in range(0,1000):
            shuffle(x)
            response = ""
            challenges = ', '.join(str(num) for num in x[:32])
            for i in range(0,32):
                if x[i] in zero_map:
                    response += "0"
                else:
                    response += "1"
            CRPs[challenge_map_index] = {'challenges': challenges,'response':response}
        
        json_string = json.dumps(CRPs)
        
        with open('CRPS.json', 'w') as file:
            file.write(json_string)
        
        serialPUF.write_json_data_toSD(json_string)
