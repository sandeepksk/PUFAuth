...
void check_command(){
  ...
  switch (command[1]) {
    ...
    case NEW_CRPS_JSON:
      ...
      myFile = SD.open("CRPS.json", O_WRITE | O_CREAT | O_TRUNC);
      Serial.println("CRPS.json opened sucessfully");

      if (myFile) {
        myFile.println(jsonStr);
        myFile.close();
        Serial.write(99);
        Serial.write(NEW_CRPS_JSON);
        Serial.write(1); // Successful write
      } else {
        Serial.write(99);
        Serial.write(NEW_CRPS_JSON);
        Serial.write(0); // Failed to open file
      }
