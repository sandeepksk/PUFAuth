...
class SerialPUF:
    ...
    def process_command(self, res, bits=[]):
        ...
        if res[1] == 40:
            ...
            self.has_reply = True
            return res[4:]
        elif res[1] == 55:  # NEW_CRPS_JSON response
            status = res[2]
            if status == 1:
                print("JSON string written to SD card successfully.")
            else:
                print("Failed to write JSON string to SD card.")
            self.has_reply = True
        else:
    ...
    def write_json_data_toSD(self,json_string):
        self.send_new_crps_json(json_string)
        r = self.check_serial_data()
